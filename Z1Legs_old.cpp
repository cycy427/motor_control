...
// V5.1 测试开关：true 时，Terminal 的 4/5/12/13 显示物理电机 RAW 数据。
// 只覆盖 motor_print_，不改变 getMotorData() 返回给上层控制的 PR 数据。
constexpr bool kV51_PrintRawAnkleMotorsInPR = false;
...
// V5.1 踝部约定：右脚为基础模型；B1=theta1，B2=theta2。
// 右脚：B1/ID12, B2/ID13，raw 角度直接进入 solver。
// 左脚：B1/ID4,  B2/ID5，直接复用同一套右脚 solver。
...
Z1Legs::AnkleA_inverse_kinematics(const YKSMotorData &pitch_joint_cmd, const YKSMotorData &roll_joint_cmd,
    ...
    // B1/theta1 命令。左右脚均直接复用右脚基础 solver；足踝路径不乘方向向量。
    ...
    cmd.ff_ = (pitch_joint_cmd.ff_ - roll_joint_cmd.ff_) / 2.0;
    return cmd;
}
...
Z1Legs::AnkleB_inverse_kinematics(const YKSMotorData &pitch_joint_cmd, const YKSMotorData &roll_joint_cmd,
    ...
    // B2/theta2 命令。左右脚均直接复用右脚基础 solver；足踝路径不乘方向向量。
    const double desired_roll = roll_joint_cmd.pos_des_;
    const double desired_pitch = pitch_joint_cmd.pos_des_;
    auto theta = inverse_kinematics(desired_roll, desired_pitch);
...
YKSMotorData Z1Legs::Pitch_forward_kinematics(const YKSMotorData &b1_motor,
    ...
    // 足踝反馈：B1/B2 raw 角度直接进入右脚基础 solver。
...
YKSMotorData Z1Legs::Roll_forward_kinematics(const YKSMotorData &b1_motor,
    ...
    const double dtheta1 = b1_motor.vel_;
    const double dtheta2 = b2_motor.vel_;
    data.vel_ = finite2(dtheta1, dtheta2) ? finiteOrZeroLocal(J[0][0] * dtheta1 + J[0][1] * dtheta2) : 0.0;

    const double tau1 = b1_motor.tau_;
    const double tau2 = b2_motor.tau_;
    data.tau_ = finite2(tau1, tau2) ? finiteOrZeroLocal(J[0][0] * tau1 + J[1][0] * tau2) : 0.0;
    ...
    if (mode_pr_ == Mode::PR) {
        ...
        for (int i = 0; i < Z1_NUM_MOTOR; ++i) {
            ...
            if (i == LeftAnklePitch) {              // 左脚 B1 / theta1 / ID4
                ...
                motor_print_[i].pos_des_ = kV51_PrintRawAnkleMotorsInPR ? motor_data_[i].pos_des_ : data[i].pos_des_;
                motor_print_[i].vel_des_ = kV51_PrintRawAnkleMotorsInPR ? motor_data_[i].vel_des_ : data[i].vel_des_;
                motor_print_[i].ff_ = kV51_PrintRawAnkleMotorsInPR ? motor_data_[i].ff_ : data[i].ff_;
                ...
                tempData = WaistA_inverse_kinematics(data[WaistPitch], data[WaistRoll]);
                motor_data_[i].pos_des_ = tempData.pos_des_;
                motor_data_[i].vel_des_ = tempData.vel_des_;
                motor_data_[i].ff_ = tempData.ff_;
                ...
                tempData = WaistB_inverse_kinematics(data[WaistPitch], data[WaistRoll]);
                motor_data_[i].pos_des_ = tempData.pos_des_;
                motor_data_[i].vel_des_ = tempData.vel_des_;
                motor_data_[i].ff_ = tempData.ff_;
                motor_print_[i].pos_des_ = data[i].pos_des_;
...
void Z1Legs::getMotorData(YKSMotorData *data) {
    ...
    if (mode_pr_ == Mode::PR) {
        ...
        if (kV51_PrintRawAnkleMotorsInPR) {
