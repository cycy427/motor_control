...
ALL_CONTROL_GIDS = sorted(LEG_GIDS + BODY_GIDS + ARM_GIDS)
...
def build_command_maps(commands):
    """
    commands must be sorted by global_id and contain every motor controlled by this script.
    Each row is: (global_id, target_pos, kp, kd)
    """
    gids = [int(row[0]) for row in commands]
    if gids != ALL_CONTROL_GIDS:
        raise ValueError(
            "ALL_MOTOR_STANCE must match ALL_CONTROL_GIDS and be sorted by global_id: "
            f"got {gids}, expected {ALL_CONTROL_GIDS}"
        )
...
if __name__ == '__main__':
    ...
    # 按 global_id 从小到大排序。每行: (global_id, target_pos, kp, kd)
    ALL_MOTOR_STANCE = [
        (0, 0.63, 500, 50),
        (1, 0.73, 500, 50),
        (2, 0.2, 500, 5),
    ...
    target_positions, stance_kp, stance_kd = build_command_maps(ALL_MOTOR_STANCE)
    ...
    try:
        ...
        command_positions(z1_leg, "leg", LEG_GIDS, leg_start, stance_kp, stance_kd)
        command_positions(z1_body, "body", BODY_GIDS, body_start, stance_kp, stance_kd)
        command_positions(z1_arm, "arm", ARM_GIDS, arm_start, stance_kp, stance_kd)
