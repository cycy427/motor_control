...
4. 检查`transmit.h`中的从站和电机数量。当前默认适配2个 EtherCAT-CANFD 从站和30个机器人电机。
5. 编译工程
    ```shell
   mkdir build
   cd build
...
完整机器人默认映射为：电机0-23接在第1个从站槽位1-24，电机24-29接在第2个从站槽位1-6。CAN ID使用全局编号，电机0-29分别对应CAN ID 1-30；硬件驱动器ID必须与该编号一致。
